...
const CountriesAdmin: React.FC<CountriesAdminProps> = ({ connectedApi }) => {
  const { t } = useLanguage();
  ...
  const handleAddCounrty = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <main className="pt-24 pb-32 px-6 max-w-screen-xl mx-auto">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100 mb-12 text-left">
        <form onSubmit={handleAddCounrty} className="flex flex-col sm:flex-row gap-4">
